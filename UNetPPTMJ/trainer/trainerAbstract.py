...
class TrainerAbstract(object):
    ...
    def checkInitialization(self) -> bool:
        ...
        for attr in attrs:
            if not hasattr(self, attr):
                warnings.warn(
                    "{class_name}.{attr_name} is not defined".format(
                        class_name=__class__.__name__, attr_name=attr
                    )
                )
                passed = False
        return passed
    ...
    def trainEpochLoop(self, dataloader: DataLoader) -> None:
        ...
        for batch, (X, y) in enumerate(dataloader):
            ...
            self.onTrainBatchEnd(
                loss=loss.item(), progress=batch * len(X), total_progress=size
            )
    ...
    def feed(
        ...
        **dataloader_params
        ...
        assert isinstance(
            train_dataset, Dataset
        ), "train_dataset has to be subclass of torch.utils.data.Dataset"
        self.train_dataset = train_dataset
        self.test_dataset = test_dataset
        self.train_dataloader = DataLoader(
            train_dataset, batch_size=self.batch_size, **dataloader_params
        )
        if test_dataset is None:
            self.test_dataloader = None
        else:
            self.test_dataloader = DataLoader(
                test_dataset, batch_size=self.batch_size, **dataloader_params
            )

    def train(self):
        assert hasattr(
            self, "train_dataloader"
        ), "Dataset not feed, use {}.feed(<params>) to feed dataset.".format(
            __class__.__name__
        )
